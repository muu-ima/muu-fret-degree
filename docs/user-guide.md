...
`Start Position`は常に`1 / e / & / a`の4コマを表示します。`Rhythm Preset`の`Quarter ×1`は拍頭へ4分音符のHit、`8ths ×2`は選択拍の`1 / &`へ8分音符のHit、`16ths ×4`は4位置すべてへ16分音符のHitを置きます。プリセットは入力補助であり、適用後も4コマから個別編集できます。選択拍の既存リズムを置き換えますが、Undoで元へ戻せます。
