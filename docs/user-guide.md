...
8分音符は開始stepから2step、付点8分音符は3stepを使います。発音後に音が続く位置は太い横線で表示され、途中へ新しいHitを置くと直前の音価はその位置までに短くなります。
...
コード譜のスラッシュ右上に音価を表示します。度数フローで複数音を生成する場合は、選択音価の範囲内で開始できる音だけを再生します。付点4分音符は6stepとして次拍の途中まで持続し、覆われた拍頭では再発音しません。Beat 4から次小節へ跨ぐ場合は次小節頭へTieを自動配置します。次小節頭をHit / Restへ変更すると、元の音価は4分までに短くなります。
