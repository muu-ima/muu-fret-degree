...
スケールやリズムの反復練習用画面です。譜面上の補助記号を見ながら、身体の動きやメトロノーム位置を確認します。
...
- ベース音は実録 `E1 / A1 / D2 / G2` サンプルから近い音源を選んで鳴らし、読み込めない場合は生成サンプルへ戻す
...
`E Major Triplets` は、E major を2オクターブで上下する波形練習です。4/4拍子で、1拍を三連符として読ませます。

- `足`: 各三連符の頭。足や身体の拍位置を意識する場所
- `メ`: 各三連符の3音目。メトロノームを意識する場所
- `〜`: 最後の1拍を省略し、同じ上下パターンが続くことを示す記号

下のEまで下降した後は、次の拍でF位置から上昇へ戻ります。E majorの調号上ではF#ですが、譜面ではFの位置に表示されます。
