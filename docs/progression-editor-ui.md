...
現在は拍頭から始まる1/16、1/8、付点1/8、1/4、1拍単位のHit / Rest / Tieに加え、選択中の拍を4分割して任意stepへHit / Restを配置できる。substepの再生と小節カード上の細分化表示は次段で接続する。付点4分は次拍の途中まで跨ぐため、イベント重複規則を拡張する段階で追加する。
