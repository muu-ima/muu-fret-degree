...
- `app/components/progression/ProgressionEditorWorkspace.tsx`: `/progression` ページのワークスペースを担当する。Sessionから進行データと更新commandを受け取り、ページヘッダー、Editor本体、操作UIを組み立てる。
...
- `app/components/progression/editor/`: Full Editorの編集UIと操作UIを置く。`ProgressionSelectionPanel` が選択中エリアの構成をまとめ、`ProgressionSelectionHeader`、`ProgressionHarmonyEditor`、`ProgressionRhythmPreset`、`ProgressionAdvancedRhythm` などの小さなUI部品へ分ける。`ProgressionHistoryActions`、`ProgressionResetDialog`、`ProgressionEditorMobileBar` はWorkspace上の履歴操作とReset操作の表示だけを担当する。
...
- `app/hooks/progression/useProgressionWorkspaceActions.ts`: `/progression` ワークスペースのUndo / Redoショートカット、Resetダイアログの開閉、Reset確定処理を担当する。
- `app/hooks/progression/useProgressionState.ts`: canonicalな進行データ、永続化、BPM同期、Undo / Redo履歴を担当する。
- `app/hooks/progression/useProgressionCommands.ts`: `useProgressionState` が使う編集commandを担当する。コード更新、リズム更新、リズムプリセット適用、配置検証を履歴commitへ変換する。
...
`ProgressionEditor.tsx` と `ProgressionEditorWorkspace.tsx` には、複雑な配列操作、選択範囲の解決、リズム配置判定、履歴ショートカット、保存形式の変換を戻さない。ここが再び大きくなった場合は、hook または editor 配下のパネルへ逃がす。
...
特定の練習目的に絞った譜面ページを担当する。

- `EMajorTripletStudy` のような単体ドリルを表示する。
- 12キー一覧ではなく、1つの反復練習を読みやすく見せることを優先する。
- ページ側はTrainingの枠、見出し、説明ラベルだけを持ち、譜面描画の細かい値は各ドリルコンポーネントへ閉じる。
- PDF保存はScalesと同じくブラウザ印刷に任せ、ページ側では`window.print()`を呼ぶボタンだけを持つ。

### `app/components/training/EMajorTripletStudy.tsx`

E Major Triplets の譜面描画を担当する。

- VexFlowのSVGレンダラーでヘ音記号、調号、拍子、音符、三連符を描画する。
- `足`、`メ`、`〜` のような練習用補助記号は、VexFlow描画後のSVGへ追加する。
- 音列、三連符グループ、補助記号の位置補正を同じコンポーネント内に置き、譜面を見ながら調整しやすくする。
- 反復の意図は複雑な反復記号に頼りすぎず、必要なら最後の1拍に`〜`を置いて簡潔に示す。

#### Training譜面レイアウトの調整方針

Training譜面では、スケール一覧よりも「1拍三連を身体で読む」ことが主目的になる。音符だけを詰め込むのではなく、三連符の数字、`足`、`メ`、調号、4/4の拍子記号が同時に読める余白を優先する。

基本方針:

- デスクトップでは1段2小節、モバイルでは1段1小節を基準にする。
- デスクトップでも、前段の`足` / `メ`と次段の三連符`3`が連なって見える場合は、補助記号を動かす前に譜面全体の高さと段間を広げる。
- モバイル幅では横方向に無理に詰めず、譜面の高さを増やして段間を確保する。
- 印刷時は画面用の横幅をそのまま使うと、モバイルから印刷した時だけ譜面が大きく詰まり、右端の音符も見切れやすい。画面用DOMとは別に固定のデスクトップ相当幅で描いた印刷用DOMを持ち、`@media print`でそちらだけ表示する。印刷用幅は広げすぎるとA4上で譜面が小さくなるため、右端の余白と音符の視認性のバランスで決める。
- 4/4や調号はVexFlowの`addTimeSignature` / `addKeySignature`で描画し、必要な場合だけSVGグループ単位で縮小・移動する。
- 拍子記号を動かす時は、音符列の開始位置も合わせて確認する。4/4を左へ寄せた分だけ、音符列にもX方向の余裕が生まれる。
- 調号と4/4は音符より左側の情報として読みたいので、音符ヘッドと重ならない範囲で小さめに扱ってよい。
- 三連符の`3`は音符や`足`と重なりやすい。X方向より先に、段の高さと上下余白を増やして解決する。
- `足`と`メ`は三連符の拍感を示す補助記号なので、音符ヘッドではなく三連符グループの頭と末尾へ揃える。
- `足`と`メ`を動かす時は、片方だけではなく両方のY位置を一緒に確認する。
- 375px幅では、カード内で音符、三連符数字、`足`、`メ`、`〜`が見切れず、下部ナビに隠れないことを最低条件にする。

調整順:

1. まずVexFlowの譜面として、小節数、音価、調号、4/4を正しく描画する。
2. 次にモバイルの段数、譜面高さ、各段のY位置を決める。
3. そのあと4/4と調号を縮小・移動し、音符開始位置を左へ寄せる。
4. 最後に`足`、`メ`、`〜`のX/Y補正を数px単位で合わせる。

この順にすると、音符の見切れを補助記号の移動だけで隠すような調整になりにくい。
...
- ベース譜面の音域は、ルートが基準Eより下に落ちる場合だけ1オクターブ上げ、2度以降もその1度より上に積む。
...
- 2オクターブ譜面は、デスクトップと印刷では1段表示を維持する。モバイル幅の一覧では1オクターブだけをプレビュー表示し、タップ時のモーダルで2オクターブ版を2段表示する。
...
- 2オクターブを狭いカード内へ横方向に詰め込まない。モバイル一覧では1オクターブ単位で読める余白を優先し、詳細確認だけモーダルへ逃がす。
